...
export default function Testimonials(){
  ...
  return (<section className="section section-black"><div className="max-w-6xl mx-auto px-6"><h3 className="text-2xl font-bold mb-6 text-white">What Our Customers Say</h3><div style={{display:'flex',alignItems:'center',gap:12}}><button onClick={()=>scroll(-1)} className="carousel-btn">◀</button><div ref={ref} style={{display:'flex',gap:'1rem',overflowX:'auto',paddingBottom:'1rem'}}>{reviews.map((r,idx)=> (<div key={idx} className="testimonial-card" style={{minWidth:300, background:'#111', color:'white'}}><p>{r.text}</p><div className="mt-4 font-semibold">— {r.name}</div><div className="mt-2" style={{color:'var(--gold)'}}>★★★★★</div></div>))}</div><button onClick={()=>scroll(1)} className="carousel-btn">▶</button></div></div></section>) }